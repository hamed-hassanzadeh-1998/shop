...
class ImageCacheService
{
    public function cache($imagePath, $size = '')
    {
        ...
        if (file_exists($imagePath)) {
            $img = Image::cache(
                function ($image) use ($imagePath, $width, $height) {
                    return $image->make($imagePath)->fit($width, $height);
                }
                , Config::get('image.default-cache-life-time')
                , true
            );
            return $img->response();
            ...
            $img=Image::canvas($width,$height,'#cdcdcd')
                ->text('Image not found',$width/2,$height/2,function ($font){
                    $font->color('#33333');
                    $font->align('center');
                    $font->valign('center');
                    $font->file(public_path('admin-assets/fonts/IRANSansWeb.woff'));
                    $font->size(24);
                });
                return $img->response();
        }
