...
class SMSController extends Controller
{
    ...
    public function update(SMSRequest $request,SMS $sms)
    {
        ...
        $inputs['published_at'] = date("Y-m-d H:i:s", (int)$realTimestampStart);
        $sms->update($inputs);
        return redirect()->route('admin.notify.sms.index')->with('swal-success','اطلاعیه پیامکی شما با موفقیت ویرایش شد');
    }
    ...
    {
        $sms->delete();
        return redirect()->route('admin.notify.sms.index')->with('swal-success','اطلاعیه پیامکی شما با موفقیت حذف شد');
    }
